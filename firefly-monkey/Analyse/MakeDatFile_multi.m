function MakeDatFile_multi(files_for_MakeDatFile_multi,T_exp)

% IN
% files_for_MakeDatFile_multi --> Or whatever the text file name is that contains all the files
% to be included. Each filename should be a row.
...
% gather info
fid = fopen('files_for_MakeDatFile_multi.txt');
filenames = textscan(fid, '%s'); filenames = filenames{1};
monk_nr = filenames{1}; monk_nr = monk_nr(1:3);
fOut = [monk_nr '_DatFile'];
