...
function isiViolations(resultsDirectory)
% [isiV percentViol clusterViol] = isiViolations(resultsDirectory)
...
%    in text file as output. 2% is default.
% Out:
...
pViol = 2; % threshold in percentage of the clusters that will be included. 

cd(resultsDirectory)
% Refractory period duration and min ISI
refDur = 0.001;
minISI = 0.001;

% % Extract the name of the csv file (old, not in use)
% d=dir('*.csv');
% d=d.name(1:8); 
...
%     end
    ...
    fprintf(1, 'cluster %3d: %d viol (out of %d spikes), Percent of Violations = %d \n', clusterIDs(c), numViolations, nSpikes, percentViol(counter));
